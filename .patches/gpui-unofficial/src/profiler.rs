...
impl TaskStatistics {
    ...
    fn add_yield_timing(&mut self, task: TaskTiming) {
        ...
        if yielded_after >= self.poll_time_to_beat {
            /* std::hint::cold_path() elided: unstable on rustc <1.93, see .patches/gpui-unofficial/README */ // most tasks are not the worst, optimize for that
            let to_replace = self
                .longest_poll_times
                .iter()
...
impl ThreadTimings {
    ...
    pub fn save_task_timing(&mut self, ended: YieldTime) {
        ...
        if trace_enabled() {
            /* std::hint::cold_path() elided: unstable on rustc <1.93, see .patches/gpui-unofficial/README */; // optimize for when the profiling is off
...
pub fn record_frame_timing(timing: FrameTiming) {
    ...
    /* std::hint::cold_path() elided: unstable on rustc <1.93, see .patches/gpui-unofficial/README */; // optimize for when profiling is off
