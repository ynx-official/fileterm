//! Storage module forked from `apps/tauri/src-tauri/src/storage/mod.rs` for the
//! GPUI runtime's G1 phase (see `docs/plans/active/gpui-refactor.md` section
//! 6.2).
//!
//! The fork is line-for-line with the Tauri source except where the GPUI
//! `AppHandle` API (see `crate::backend::app_handle`) differs from Tauri's
//! framework handle:
//! - `use tauri::{AppHandle, Manager}` → `use crate::backend::app_handle::AppHandle`
//! - `use crate::AppError` → `use crate::error::AppError`
//! - `app.path().app_data_dir().map_err(...)?` → `app.app_data_dir_result()?`
//!   (GPUI's `AppHandle` resolves the data dir at construction time, so the
//!   `Result`-returning variant never errors — kept only so the fork's `?`
//!   operators compile without rewrite.)
//! - `app.path().app_config_dir().ok()` → `None` (GPUI's `AppHandle` only
//!   tracks the data dir, not the config dir; `select_legacy_directory`'s
//!   config-parent candidate branch is simply skipped.)
...
pub fn state_path(app: &AppHandle) -> Result<PathBuf, AppError> {
    let dir = app.app_data_dir_result()?;
...
pub fn migrate_legacy_data_once(app: &AppHandle) -> Result<(), AppError> {
    let current_dir = app.app_data_dir_result()?;
