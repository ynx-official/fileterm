...
- `ssh2`：SSH shell、SFTP
- `basic-ftp`：FTP
- `electron-builder`：桌面打包
- `safeStorage` + 系统钥匙串策略：敏感信息存储

### 数据与持久化

- SQLite
- Drizzle ORM 或轻量自定义 repository 层
...
建议直接采用 workspace monorepo 结构。当前仓库首轮骨架先使用 `npm workspaces`，后续如需切换到 `pnpm` 也不会影响目录设计：
