...
当前运行基线已经切换为 Tauri v2；Rust + Tauri 实现位于 `apps/desktop/src-tauri`。历史 Electron 42.4.0 代码仍保留用于迁移对照和旧数据兼容，但当前 `apps/desktop/package.json` 与 lockfile 已不再声明 Electron 运行时。Tauri 已完成 Phase 0–4 的代码/contract 主体与本机协议夹具，尚待真实服务、实体设备、三平台 CI 和打包 UI 的发行候选验收；实现状态不能直接等同于正式发布就绪。
...
- Tauri bridge 位于 `apps/desktop/src/bridge/tauri-api.ts`，Rust commands/services/sessions 位于 `apps/desktop/src-tauri/src/`。
...
历史 Electron-only 依赖（`ssh2`、`basic-ftp`、`serialport`、`electron-builder`）仍可在遗留源码或历史文档中看到，但不属于当前 Tauri runtime 的依赖。
...
- macOS 菜单栏托盘图标使用 `apps/desktop/build/trayTemplate*.png` template 资源，由 Rust/Tauri backend 设置 template 属性。
...
- **遗留 Electron controller 测试**：`test/controllers/` 仍依赖 `dist-electron/main` 产物，是历史实现对照，不是当前 Tauri 验收证据。Tauri 代码以 Rust unit/integration/contract test 和发行候选清单为准；后续应清理或明确隔离这组遗留测试。
