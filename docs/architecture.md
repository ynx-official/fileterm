...
当前运行基线仍是 Electron；Rust + Tauri 实现位于 `apps/desktop/src-tauri`，正在作为单运行时迁移目标推进。Tauri 已完成 Phase 0–3 的实现，Phase 3 尚待真实服务的三平台验收；Electron 的 Phase 4 协议扩展和 Transfer 实现不能直接视为 Tauri 已完成能力。
...
- Tauri 当前未完成 Transfer、FTP/FTPS、Telnet、Serial、WebDAV 真实同步和发行切换；Phase 3 的 SSH 网络能力仍需要真实服务和三平台验收。这些项目仍属于迁移计划，不应从 Electron 基线能力清单推断为 Rust 已支持。
