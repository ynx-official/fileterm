...
当前运行基线仍是 Electron；Rust + Tauri 实现位于 `apps/desktop/src-tauri`，正在作为单运行时迁移目标推进。Tauri 已完成 Phase 0–4 的代码/contract 主体与本机协议夹具，尚待真实服务、实体设备、三平台 CI 和打包 UI 的发行候选验收；实现状态不能直接等同于正式发布就绪。
...
- Tauri 当前已覆盖桌面壳、JSON 存储、Workspace snapshot，以及 russh SSH shell/SFTP/MFA/host verification、系统指标、CWD 跟随、重连水化、自动重连、远程编码、递归 chmod、单级 Jump Host、SOCKS5/HTTP CONNECT 代理和运行时 SSH `-L/-R/-D` 隧道。
...
- 原生关闭快捷键由 main process 统一收口：macOS 使用 `Cmd+Q` 请求应用退出确认、`Cmd+W` 请求关闭当前工作区项/子窗口；Windows/Linux 分别保持 `Alt+F4` 退出与 `Ctrl+W` 关闭窗口语义。托盘退出和窗口关闭必须走同一确认与 transfer journal 清理链路。
