...
- 暂停、继续、丢弃断点。
- 应用重启后恢复任务列表，并将未完成运行态转换为“已中断，可继续”。
- 文件大小与源文件身份校验。
- 目录逐文件 manifest，完成文件复核后跳过，当前文件按实际断点长度恢复。
- 可重试连接错误采用 0.5s / 1s / 2s 退避；连接重新打开后自动恢复 interrupted 任务。
- SSH root/shell fallback 字节续传，保留 `/tmp` staging 以兼容目标目录权限边界。
...
- 协议测试夹具覆盖 SFTP 双向 offset、符号链接收尾、FTP APPE、REST/STOR、下载 REST、rename、显式 FTPS 和隐式 FTPS；受限环境缺少 sshd/openssl 或不能监听 localhost 时相应用例会明确标记 skipped。
