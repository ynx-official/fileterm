...
| 文档版本   | v0.3（拆分草案，待评审）                                                            |
| 更新日期   | 2026-07-18                                                                          |
| 状态       | G1-G5 代码层全部落地（127 测试 + clippy 全绿）；真机验收与 bridge 接入待开工         |
| 编写人     | 架构组                                                                              |
| 适用分支   | `gpui`                                                                              |
| 仓库根目录 | `/workspace`                                                                        |
...
| 2026-07-18 | G1 存储 fork 完成：`apps/gpui/src/backend/` 落地 `app_handle.rs`（持有 `PathBuf data_dir`，替代 `tauri::AppHandle`）、`storage/` 全套原子读写、`commands/workspace_window.rs`（input struct + `PLACEMENTS_CHANGED_EVENT` 常量 line-for-line 镜像 Tauri IPC contract，bridge surface 返回 Unsupported）。`FileTermDesktopApi` trait 的 G1 方法签名全部就位 |
| 2026-07-18 | G2 多窗口完成：`window/registry.rs`（三张 `RwLock<HashMap>` 跟踪 detached_tabs / tab_owner / handles + `detach_tab` / `return_tab_to_main` / `return_tabs_to_main` / `return_single_tab_to_main` / `list_placements` / `register_handle`，8 单测覆盖）、`window/kind.rs`（7 种 `WindowKind` + `as_str` / `holds_tabs` / `uses_client_decorations` / `label_for`，3 单测）、`window/menu.rs`（5 个 `gpui::Action` menu + `Menu::new().items()` builder API，3 单测）、`window/tray.rs`（tray menu builder + 2 单测） |
| 2026-07-18 | G3 SSH + 终端 dock 完成：`ssh/controller.rs`（`SshController` 生命周期 stub + 1 单测，使用 `russh` *）、`ssh/terminal_dock.rs`（`PathCompleter` 远端路径补全 stub + 5 单测，`#[allow(dead_code)]` 标 G3 stub 字段）、`ssh/system_sidebar.rs`（`SystemMetricsCollector` Linux/BusyBox/Windows 多平台采集 + CRLF 归一化 + `parts.first()` 加固，6 单测） |
| 2026-07-18 | G4 SFTP + 文件传输完成：`sftp/client.rs`（`SftpClient` 上传/下载/列目录/删除/重命名 stub + 1 单测，`#[derive(Debug)]` 修复 `unwrap_err()` 约束，使用 `russh-sftp` *）、`sftp/file_manager.rs`（`SftpFileManager` 高层操作 + 4 单测）、`sftp/transfer.rs`（`TransferService` 全套：`TransferDirection` / `TransferTaskStatus` 8 状态分区 + `active()` / `terminal()`；`TransferTask` / `TransferManifest` / `TransferJournal` 字段对字段镜像 Tauri `services/transfers.rs` 以保证 `transfer-journal.json` 双 runtime 互操作；常量 `JOURNAL_VERSION=1` / `PARTIAL_SUFFIX=".fileterm-part"` 用单测锁定；5 单测覆盖状态分区、空 stub、enqueue/pause/resume/cancel 返回 Unsupported、常量锁定） |
| 2026-07-18 | G5 可拆分窗口 + 拖拽 + 打包完成：`window/detach.rs`（`detach_tab_to_new_window` 纯逻辑半 + `return_window_to_main`，5 单测）、`window/tab_drag.rs`（`TabDragState` 状态机：`start` 返回 bool 避免 clippy::result_unit_err、`finish` 内部 `take()` 清空 active、`DragDropTarget` 三态分类、`ScreenBounds::contains` 左上含右下排，8 单测覆盖 start+finish/start 两次失败/cancel/drop 到其他 window/drop 到空屏幕/无 start 的 finish/z-order 顶层优先/bounds 边界）、`window/registry.rs` 增 `return_tab_to_main` 单 tab 回流 + 2 单测、`backend/commands/workspace_window.rs` 增 3 个 pure-logic helper（`move_tab_with_registry` / `detach_tab_with_registry` / `finish_tab_drag_with_registry`，关键 bug 修复：先抓 `tab_id` 再 `drag.finish` 否则 `take()` 后丢失；6 新单测覆盖 mint 新 window id / move 到 main 留兄弟 / move 到其他 window / drag SameWindow no-op / OtherWindow 移动 / NewWindow detach / 无 active drag 返回 None）、`build.rs`（macOS Info.plist + Windows manifest 写 `OUT_DIR`）、`scripts/build-macos.sh` / `build-windows.sh` / `build-linux.sh` 三平台打包脚本（codesign / notary / NSIS / AppImage 全部 gated by 环境变量） |
| 2026-07-18 | **G1-G5 代码层全部落地**：`cargo test -p fileterm-gpui --lib` **127 passed / 0 failed / 0 ignored**（0.02s）；`cargo clippy -p fileterm-gpui --all-targets --all-features -- -D warnings` 全绿。bridge 层仍返回 `Unsupported`（依赖 `&mut App` 的 GPUI 窗口开启与 placement 广播），pure-logic 半全部 `pub` 暴露供 `view::workspace` 层接入 |
| — | G1-G5 真机验收待开工：bridge 层从 `Unsupported` 接入真实 GPUI `cx.open_window(...)` + 广播 `PLACEMENTS_CHANGED_EVENT`；`view::workspace` 层落地；5.1 门禁 5 项真机验收；G-1.7 IME + selection + cursor blink；OSC 52/1337 |
