...
- [x] 密钥管理器与连接/命令管理器统一表体视觉：移除独有的逐行横隔线和嵌套底色，文件夹行仅保留
      展开箭头，密钥实体与“全部密钥”入口使用本地 key 图标。
- [x] 命令编辑器保存/取消按钮与确认弹窗统一动作规格；SSH 私钥导入弹窗拆出 `#242424` 紧凑标题表头
      并补本地 key 图标。
- [x] 系统资源侧边栏折叠偏好改为按会话标签保存；焦点模式和资源不可用只做临时强制折叠，
      不再覆盖用户偏好或串扰新标签。
