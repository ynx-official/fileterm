...
FileTerm 是一个基于 Electron 42 + React + TypeScript 的桌面远程工作台，支持 SSH/SFTP、FTP/FTPS、Telnet、Serial 四类协议。当前后端（Electron 主进程）约 50 个 TS 文件、总计约 8800 行核心代码，承载了协议会话、文件传输、workspace 运行时、系统指标采集、JSON 文件存储等能力。
...
| 维度     | 现状                                                                           |
| -------- | ------------------------------------------------------------------------------ |
| 桌面壳   | Electron 42（`package.json` 顶层依赖 `electron@^42.4.0`）                      |
...
TS 侧 `app-update-service.ts`（electron-updater）。Rust 侧用 **Tauri v2 updater plugin**（`@tauri-apps/plugin-updater` + Rust 端），签名/公证在 Phase 5 处理。`AppUpdateStatus` 结构与 `@fileterm/core` 一致，adapter 层映射。
...
**风险：** russh 某平台 PoC 失败（回退 ssh2 crate）；keyboard-interactive 异步回调与「请求-响应」配对的死锁/超时；jumphost stream forwarding 实现复杂度。
...
**验收：** 协议测试、controller 测试、传输协议测试、真实设备手测全部通过。
...
| M5.4 迁移工具 + 回滚          | 用户数据迁移幂等+备份；失败可回滚 Electron      |
...
3. **阶段级回滚：**
   - Phase 1-2 失败：切回 Electron，Rust 存储产物与 TS 格式一致，可互读。
   - Phase 3（SSH）失败：保留已完成的 Tauri 基础能力，SSH command 明确返回 unsupported，继续推进 Rust 实现。
   - Phase 5 灰度失败：停止 Tauri 灰度，用户回滚到 Electron 版本，数据兼容。
4. **配置开关：** adapter 选择由构建时 env 决定（`FILETERM_BACKEND=electron|tauri`），支持快速切换。
...
1. **russh vs ssh2 最终锁定**：需三平台 PoC 结果后定。若 russh 在 Windows 上 ssh-agent (`SSH_AUTH_SOCK`/pageant) 支持不完善，可能需混合方案或回退 ssh2 crate。
2. **Monaco/xterm worker 在 Tauri 的资源加载**：Vite 构建产物在 Tauri webview 下的 `asset`/`crossOrigin` 配置需实测，可能需调整 base path。
3. **Tauri updater 与现有 electron-updater 的更新通道共存**：灰度期是否两套更新通道并行？需产品决策。
