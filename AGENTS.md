...
- Electron main process：`apps/electron/src/main`
- Electron IPC 注册：`apps/electron/src/main/ipc/`（按领域拆分）
- Electron 业务服务层：`apps/electron/src/main/services`
- Electron preload：`apps/electron/src/preload/preload.cts`
...
- Renderer hooks：各 app 自己的 `src/renderer/hooks/`；不得跨 app 引用
...
- Layout、ErrorBoundary、工作区、终端和主题组件：分别位于 `apps/tauri/src/renderer/` 与 `apps/electron/src/renderer/`，不得跨 app 复用。
- 领域类型：`packages/core`
...
- `apps/electron/src/main/ipc/` 与 `apps/electron/src/main/services/`：Electron 的 IPC、协议与传输实现。
- `apps/electron/src/renderer/`：Electron 专用 UI。
...
| Electron 测试     | `npm run test:electron`                                 | Electron unit/controller 测试   |
| Tauri 测试        | `npm run test:tauri`                                    | Rust unit + contract 测试       |
| Electron 协议测试 | `npm run test:transfers:protocol -w @fileterm/electron` | 需先 build core + Electron main |
